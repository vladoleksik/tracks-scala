object TwoGoodNeighboursRule extends InferenceRule {

  //This rule fills in the track in a cell if it is certain to have one and there are only two possible directions for the track to connect.

  def apply(puzzle: Puzzle): Puzzle = {
    val newPuzzle = Puzzle(puzzle)
    newPuzzle.state.foreach((row: Array[Cell]) => (row.foreach((cell: Cell) => (
      if (cell.isTrackCertain && !cell.isTrackKnown) {
        var ct: Int = 0
        var track: Int = 0
        if (cell.row > 0) {
